...
func (m Model) Update(msg tea.Msg, p *tea.Program) (tea.Model, tea.Cmd) {
	switch msg := msg.(type) {
	case tea.KeyMsg:
		if m.IsStreaming {
			return m, tea.Batch()
...
func sendAPIRequest(m Model, p *tea.Program) {
	if p == nil {
...
func main() {
	...
	p := tea.NewProgram(model, tea.WithUpdate(model.Update))
	p.Use(tea.InputDelegate{
		IgnoreCtrlC: true,
	})
